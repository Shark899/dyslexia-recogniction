...
def thirdTest():
    ...
    button1Choose = Button(frame1, text="Prima Scheda", command=lambda:btn1Choose())
    button1Choose.grid(row=2, column=0)

    button2Choose = Button(frame1, text="Seconda Scheda", command=lambda:btn2Choose())
        ...
        def submit(value):
            ...
            if len(list2) < 10:
                ...
                if score >= 8 and score <= 10:
                    ...
                    lab = Label(frame1,
                                text=f"Your score is {score}/{len(list2)}. Probabilmente sei un ragazzo dislessico")
                    lab.grid(row=5, column=0)
                else:
...
firstTest = Button(frame1,text="Text Test",command=firstTest)
firstTest.grid(row=0,column=1)
secondTest = Button(frame1,text="Speech Test",command=secondTest)
secondTest.grid(row=0,column=2)
thirdTest = Button(frame1,text="Schede",command=thirdTest)
thirdTest.grid(row=0,column=3)
